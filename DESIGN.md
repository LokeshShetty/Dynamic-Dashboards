...
| The table is not virtualised                           | It paginates, so 8 to 100 rows are in the DOM. A virtualiser would add a dependency and a scroll container to save nothing.                        | Windowing rows and columns, once a wide table is measured rather than imagined. `SELF_REVIEW.md` item 4 has the case. |
